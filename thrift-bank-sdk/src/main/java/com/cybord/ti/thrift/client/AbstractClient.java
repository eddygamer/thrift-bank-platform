...
public abstract class AbstractClient {
	...
	public AbstractClient(String url, int maxRetries, int delay, TimeUnit unit) {
		this.url = url;
		setDefaultRetryPolicy(maxRetries, delay, unit);
	}
	...
	/**
	 * Execute a DELETE request over the endpoint received with the content type
	 * specified and sending the object received in the body
	 *
	 * @param endpoint
	 *            Endpoint will be executed
	 ...
	 * @return A response object with the entity response and the Http status
	 */
	protected Response delete(String endpoint,String type) {
		WebTarget client = createClient(endpoint);
		Optional<RetryPolicy> result = getDefaultRetryPolicy();
		if (result.isPresent()) {
			return Failsafe.with(result.get())
					.get((response)-> client.request(type).delete());
		} else {
			return client.request(type).delete();
		}
	}
	...
	public void setDefaultRetryPolicy(int maxRetries, int delay, TimeUnit unit) {
		defaultRetryPolicy = 
				new RetryPolicy()
				.retryIf((Response resp) ->{ if(resp!=null) { 
					return resp.getStatusInfo().getFamily() == Status.Family.SERVER_ERROR;				
						}else {return false;//no retry if an exception happen
						}})
